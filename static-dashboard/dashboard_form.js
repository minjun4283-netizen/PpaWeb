  ...
  try {
    ...
    async function rebuildDashboard() {
      ...
      try {
        ...
        rebuildBtn.textContent = "대시보드 새로고침";
      }
    }
    ...
    async function saveCurrentTable() {
      ...
      if (!pkName || !record[pkName]) {
        alert((pkName || "PK") + "는 필수입니다.");
        return;
      }

      saveBtn.disabled = true;
      saveBtn.textContent = "엑셀에 저장 중...";

      try {
        var data = await apiPost("/api/save", { table: tableName, record: record });
    ...
    modeSel.addEventListener("change", function () {
      renderMode(true).catch(function (e) { alert("목록 로딩 실패\n" + (e.message || e)); });
    });
