...
`T_수요기업`, `T_판매계약`, `T_전기사용지`, `T_구매계약`, `T_수급매칭` 5개 표는
실제 xlsm 화면 캡처로 컬럼을 확인하고 반영했습니다 (`backend/src/schema/tableDefs.ts`
참고). 검증 열(PK중복/PK공란/*참조/조합중복)은 이 시스템이 저장하지 않고 그때그때
계산하므로 스키마에서 제외했습니다.

### ⚠️ `T_발전소`만 아직 추측 스키마입니다

`T_발전소`는 VBA 코드에 PK 컬럼명(`발전소ID`)과 툴팁 로직이 시도하는 후보
컬럼명(발전소명/발전소명칭/시설명, 발전원유형/발전원/에너지원/발전유형,
설비용량 등)만 드러나 있어 정확한 컬럼을 알 수 없었고, 재생에너지 PPA 도메인
지식으로 합리적으로 채워둔 추측값입니다. 실제 xlsm의 `T_발전소` 화면 캡처를
공유해주시면 나머지 5개 표처럼 정확히 맞추겠습니다.

**실제 운영에 쓰기 전에** 로그인 후 관리자 계정으로 **컬럼관리** 화면
(`/admin`)에서 `T_발전소`를 실제 엑셀 표의 컬럼명과 비교해 확인해주세요. 컬럼은
