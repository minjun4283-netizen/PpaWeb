...
const SOON_DAYS=180;
/* 매칭률(구매 대비 매칭 완료율) KPI가 경고색으로 바뀌는 기준 - 이
   아래로 떨어지면 매칭 안 된 구매계약이 상당히 쌓여 있다는 뜻입니다. */
const MATCH_RATE_WARN_PCT=70;
...
/* ── 홈: 년월별 추이 ────────────────────────────────────────────────────
   패널 하나에 세 구역을 위→아래로 쌓습니다:
   1) 매칭된 계약·발전원별 누적 용량 (월별, 2020년~, 신규 - 사용자 확정 로직)
   2) 분기별 신규·종료 판매계약 용량 비교 (신규)
   3) 지표별 상세 추이 - 예전부터 있던 5가지 지표 선택 + 건수/용량 전환 +
      기간 지정 + 일별 드릴다운 (2)번 작업 때 실수로 지워졌던 걸 복구함)
   세 구역 다 각자 데이터가 없으면 그 구역만 안내 문구로 대체되고, 나머지
...
/* 2020-01부터 시작하는 월 목록 - 실제 데이터가 2020년 이전에도 있더라도
   ...
   종료일 하나 때문에 축이 끝없이 늘어지지 않도록 하는 안전장치. */
function monthRangeFrom2020(maxI){
  const minI=ymIdx(TREND_BASE_YEAR+'-01');
  if(maxI<minI) maxI=minI;
  const clamped=(maxI-minI)>359;
  if(clamped) maxI=minI+359;
  const out=[];for(let i=minI;i<=maxI;i++) out.push(idxYm(i));
  return {keys:out,clamped};
}
function cumulativeSeries(perKeyMap,keys){
  let running=0;
...
function saleDateMaps(){
  const S=byKey['T_판매계약'];const newM={},endM={};
...
/* '매칭된 계약' 누적 용량 - 수급매칭 한 건마다, 그 매칭에 걸린 구매계약
   용량과(전기사용지를 거쳐 연결된) 판매계약 용량 중 작은 값을 그 매칭의
   실제 공급 가능 용량으로 보고 합산합니다(사용자 확정 규칙:
   MIN(판매계약 용량, 구매계약 용량)). 기준 시점은 그 판매계약의 계약일
   (비어 있으면 공급기한_판매)로 삼습니다 - 수급매칭 자체엔 날짜 컬럼이
   없고, 매칭은 결국 수요(판매계약) 쪽에서 성립되는 일이기 때문입니다.
   2020년 이전은 집계에서 제외합니다. 발전원별 계열은 이 값을 그대로,
   그 매칭에 쓰인 구매계약의 발전소가 속한 발전원별로 나눠 합산한
   것입니다 - 전체 합계와 같은 재료를 다른 각도로 보여주는 것뿐이라 두
   계열의 마지막 값들을 더하면 총합과 (반올림 오차 정도만 빼고) 같아야
   합니다. */
function matchedCapacityByMonth(excludeBuyIds){
  excludeBuyIds=excludeBuyIds||new Set();
  const M=byKey['T_수급매칭'],B=byKey['T_구매계약'],E=byKey['T_전기사용지'],S=byKey['T_판매계약'],P=byKey['T_발전소'];
  const totalM={},bySource={};
  if(!M||!B||!E||!S) return {totalM,bySource};
  const buyCap={},buyPlant={};
  B.rows.forEach(r=>{
    buyCap[r.cells['구매계약ID']]=Number(r.cells['구매계약용량(MW)']);
    buyPlant[r.cells['구매계약ID']]=r.cells['발전소ID'];
  });
  const siteToSale={};
  E.rows.forEach(r=>{siteToSale[r.cells['전기사용지ID']]=r.cells['판매계약ID'];});
  const saleCap={},saleDate={};
  S.rows.forEach(r=>{
    const pk=r.cells['판매계약ID'];
    saleCap[pk]=Number(r.cells['판매계약용량(MW)']);
    saleDate[pk]=r.cells['계약일']||r.cells['공급기한_판매'];
  });
  const plantSource={};
  if(P) P.rows.forEach(r=>{plantSource[r.cells['발전소ID']]=String(r.cells['발전원']||'').trim()||'기타';});

  M.rows.forEach(r=>{
    const buyId=r.cells['구매계약ID'],siteId=r.cells['전기사용지ID'];
    if(excludeBuyIds.has(String(buyId))) return;
    const saleId=siteToSale[siteId];
    if(!(buyId in buyCap)||!saleId||!(saleId in saleCap)) return;
    const bc=buyCap[buyId],sc=saleCap[saleId];
    if(isNaN(bc)||isNaN(sc)) return;
    const ym=monthKey(saleDate[saleId]);
    if(!isYmFromBaseYear(ym)) return;
    const matched=Math.min(bc,sc);
    totalM[ym]=(totalM[ym]||0)+matched;
    const src=plantSource[buyPlant[buyId]]||'기타';
    (bySource[src]=bySource[src]||{})[ym]=(bySource[src][ym]||0)+matched;
  });
  return {totalM,bySource};
}
...
/* 예전부터 있던 지표별 상세 추이(5개 지표 선택 + 건수/용량 전환 + 기간
   직접 지정 + 월 막대를 눌러 일별로 파고들기) - 위 두 신규 구역을 추가
   하면서 실수로 코드째 지워졌던 기능입니다. 그대로 복구해 세 번째
   구역으로 씁니다(패널을 감싸지 않고 안쪽 HTML만 돌려줌). */
const TREND_METRICS={
...
function tHome(){
  ...
  /* 매칭률 = 수급매칭으로 실제 연결된(MIN 매칭 규칙) 누적 용량 ÷ 구매계약
     유효 용량. matchedCapacityByMonth()는 '누적 현황' 콤보차트와 같은
     집계 로직이지만, 분모(purchMW)가 이미 "현황이 전부 종료류인 구매
     계약"을 뺀 값이므로 분자도 그런 구매계약(purchTermIds)에 걸린
     매칭은 제외해야 100%를 넘는 이상한 비율이 나오지 않습니다 - 콤보
     차트는 종료 여부와 무관하게 전체 매칭 이력을 보여주는 목적이라 이
     제외를 적용하지 않으므로, 두 화면의 숫자가 다를 수 있습니다(의도된
     차이). */
  const totalMatched=Object.values(matchedCapacityByMonth(purchTermIds).totalM).reduce((s,v)=>s+v,0);
  const matchRatePct=purchMW>0?totalMatched/purchMW*100:null;
  const unmatchedMW=Math.max(0,purchMW-totalMatched);
  /* 하나의 구매계약이 여러 전기사용지/판매계약에 나눠 매칭되는 "분할
     매칭"에서는 MIN(구매,판매)를 매칭 건별로 그대로 더하기 때문에(이미
     콤보차트가 쓰는 계산 방식을 그대로 재사용 - 그 로직 자체는 이번
     범위에서 손대지 않음), 합계가 구매계약 용량 자체를 넘어설 수
     있습니다. 100%를 넘는 이상한 숫자를 그대로 보여주는 대신 넘었다는
     사실 자체를 안내로 바꿔 보여줍니다. */
  const overMatched=matchRatePct!==null&&matchRatePct>100.5;
  ...
  return `<section>${printHead('PPA 계약관리 현황 요약','')}
    ...
    ${kpi('매칭률 (구매 대비)'+infoTip('구매계약 유효 용량(현황이 전부 "종료"류인 계약 제외) 대비, 그 유효 계약들에 걸린 수급매칭으로 실제 연결된(구매·판매 중 작은 쪽 기준) 누적 용량의 비율입니다. 하나의 구매계약이 여러 곳에 나눠 매칭된 경우 100%를 넘을 수 있습니다.'),
      matchRatePct===null?'—':(overMatched?'100%+':nf(matchRatePct,0)+'%'),
      matchRatePct===null?'구매계약 없음':(overMatched?`매칭 ${nf(totalMatched)}MW · 구매계약 용량 초과(분할 매칭)`:`매칭 ${nf(totalMatched)}MW · 미매칭 ${nf(unmatchedMW)}MW`),
      (matchRatePct!==null&&(overMatched||matchRatePct<MATCH_RATE_WARN_PCT))?'warn':'',
      "state.tab='T_수급매칭';render()")}
